...
public class ChatClient extends JFrame implements ActionListener, Runnable{
	...
	JPanel centerPane = new JPanel(new BorderLayout());
		JPanel connPane = new JPanel(new BorderLayout());
			JTextField connTf = new JTextField("192.168.0.2"); // 본인 IP 적기
	...
	public ChatClient() {
		...
		// all message
		centerPane.add(sp);
		msgTa.setBackground(Color.DARK_GRAY);
		msgTa.setForeground(Color.WHITE);
	...
	public void actionPerformed(ActionEvent ae) {
		...
		if(eventObj == connTf || eventObj == connBtn) {
			setServerConnection(); // 서버에 접속, IO 생성
	...
	public void setServerConnection() {
		try {
			...
			s = new Socket(ia, 19000); // 이 시점에 접속 완료.
			// IO 생성
			br = new BufferedReader(new InputStreamReader(s.getInputStream()));
			pw = new PrintWriter(new OutputStreamWriter(s.getOutputStream()));
			
			connTf.setEnabled(false); // 다시 접속 못 하게 비활성화
			...
			System.out.println("서버접속 에러 발생" + e.getMessage());
		}
	}
			...
			try {
				...
				if(receiveData != null && !receiveData.equals("")) {
					...
					if(receiveData.substring(0, 4).equals("*MG$")) {
						...
						connModel.removeAllElements(); // 모델의 모든 목록 지우기
