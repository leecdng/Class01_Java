...
public class ddd extends JPanel implements FocusListener, ActionListener, ItemListener{ // 은정
	...
	// 등록/수정 전 입력 내용 검사
	public void checkInfo(String evtStr) {
		if(tf[0].getText()==null || tf[0].getText().equals("")) {
			...
			// 다 잘 입력됐으면
			if(chk==true && evtStr.equals("등록")){
				insertCenter();
			} else if(chk==true && evtStr.equals("수정 완료")){
				updateCenter();
			}
		}
	}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(evt instanceof JComboBox) {
			...
			if(evtStr.equals("등록")) {
				checkInfo("등록"); // 내용 잘 입력했나 체크 --- 메서드 안에 진료소 정보 등록하는 메서드 있음
				/////////////////////////////////////// 등록 JOptionPane에서 확인 누르면 화면 전환
			} else if(evtStr.equals("수정 완료")) {
				checkInfo("수정 완료"); // 내용 잘 입력했나 체크 --- 메서드 안에 진료소 정보 수정하는 메서드 있음
