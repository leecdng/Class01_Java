...
//				System.out.println(vo.getAdmin_id());
   ...
   public AdminVO setMyInfo(String admin_id) {
      ...
      try {
         ...
         String sql="select admin_id,admin_pw,admin_name,admin_num,admin_tel, admin_local from admindata where admin_id=?";
         pstmt=conn.prepareStatement(sql);
         pstmt.setString(1, admin_id);
         rs=pstmt.executeQuery();
   ...
   //희수 추가
   //로그인 체크
   public AdminVO logincheck(String admin_id) {
      AdminVO vo = new AdminVO();
      try {
         //1. db연결
         dbConn();
         ...
         System.out.println("관리자 모드 : 아이디와 일치하는 정보 불러오기 오류");
         e.printStackTrace();
      }finally {
         dbClose();
      }
   ...
   public String searchId(String name, String num ,String tel){
      ...
      try {
         ...
         System.out.println("회원 ID 찾기 오류 발생");
         e.printStackTrace();
      }finally {
         dbClose();
      }
   ...
   public int passWordModify(String Admin_pw, String Admin_id) {
	   ...
	   try {
	      dbConn();
	      String sql = "update admindata set user_pw=? where user_id=?"; //쿼리문 문제 없음 정상임
	      pstmt = conn.prepareStatement(sql);
